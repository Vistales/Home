...
df <- df %>%   
  ...
  summarise(remit = sum(Observation.Value))   ### plotting the remittance of each country for all years 
plot_ly(df, x = ~year, y = ~remit, color = ~country,                   type = "scatter", mode = "lines+markers") %>%       
